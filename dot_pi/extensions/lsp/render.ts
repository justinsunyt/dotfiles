...
import { type ToolRenderResultOptions, getLanguageFromPath, Theme, truncateHead, DEFAULT_MAX_LINES } from "@mariozechner/pi-coding-agent";
import { Text } from "@mariozechner/pi-tui";
import { highlight, supportsLanguage } from "cli-highlight";

// Local truncation helpers
const TRUNCATE_LENGTHS = { symbols: 30, references: 20, diagnostics: 25 };
function truncate(s: string, max: number): string {
  return s.length > max ? s.slice(0, max - 1) + "…" : s;
}
function formatExpandHint(theme: Theme, expanded: boolean): string {
  return expanded ? "" : theme.fg("muted", "(Ctrl+O to expand)");
}
function formatMoreItems(count: number, theme: Theme): string {
  return theme.fg("muted", `... and ${count} more`);
}
