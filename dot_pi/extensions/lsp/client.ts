...
async function writeMessage(
	...
	const fullMessage = Buffer.concat([Buffer.from(header, "utf-8"), contentBytes]);
...
async function startMessageReader(client: LspClient): Promise<void> {
	...
	const processData = async (value: Buffer) => {
		// Atomically update buffer before processing
		const currentBuffer = concatBuffers(client.messageBuffer, new Uint8Array(value));
...
export async function getOrCreateClient(config: ServerConfig, cwd: string, initTimeoutMs?: number): Promise<LspClient> {
	...
	const clientPromise = (async () => {
		...
		// Wrap Node's ChildProcess to match our interface
		const proc = {
			...
			exitPromise: new Promise<number | null>((resolve) => {
				...
				childProc.on("error", () => resolve(null));
			}),
		};
