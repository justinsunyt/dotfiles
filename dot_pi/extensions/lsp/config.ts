...
import { createSwiftLintClient } from "./clients/swiftlint-client";
...
function applyRuntimeDefaults(servers: Record<string, ServerConfig>): Record<string, ServerConfig> {
	...
	if (updated.swiftlint) {
		updated.swiftlint = { ...updated.swiftlint, createClient: createSwiftLintClient };
	}
...
function getConfigPaths(cwd: string): string[] {
	...
	// Project config directories (.pi/)
	const projectDirs = [join(cwd, ".pi")];
	...
	// User config directories (~/.pi/agent/)
	const userDirs = [join(homedir(), ".pi", "agent")];
